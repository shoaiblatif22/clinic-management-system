...
export function ResetPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // ponytail: token stays as query param (arrives from email link); password moves to body to avoid URL logging
            const response = await fetch(
                `http://localhost:8081/user/api/v1/password-reset/reset?token=${encodeURIComponent(token)}`,
                {
                    method: 'POST',
            ...
            if (!response.ok) {
                ...
                throw new Error((errorData as { message?: string }).message || 'Failed to reset password');
            }
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
                <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
                    ...
                    <h2 className="mt-2 text-2xl font-bold text-gray-900">Password Reset Successful!</h2>
                    <p className="mt-2 text-gray-600">Your password has been successfully updated. Redirecting to login...</p>
                    <div className="mt-6">
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                <Link to="/login" className="inline-flex items-center text-teal-600 hover:text-teal-700 mb-6">
                    <ArrowLeft className="w-4 h-4 mr-2" />
                    Back to Login
                </Link>
                <h1 className="text-center text-3xl font-bold text-gray-900 mb-2">Reset Your Password</h1>
                <p className="text-center text-gray-600 max-w-sm mx-auto mb-8">Please enter your new password below</p>
            </div>
                            ...
                            {formData.password && (
                                <div className="mt-2">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">
                                        Password strength: {getPasswordStrength(formData.password) || 'none'}
                                    </p>
                                </div>
                            )}
                        </div>
                            ...
                            <ul className="space-y-2">
                                {[
                                    ...
                                    <li key={index} className="flex items-center text-sm text-gray-600">
                                        <Check className={`w-4 h-4 mr-2 ${requirement.met ? 'text-green-500' : 'text-gray-300'}`} />
                                        {requirement.text}
                                    </li>
                                ))}
                            </ul>
